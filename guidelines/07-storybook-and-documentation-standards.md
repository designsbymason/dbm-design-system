...
1. **Overview** (added 2026-07-27, revised same day) — title + tier badge via `<ComponentHeader>`, then the one-paragraph description, with **Import as a nested `### Import` subsection right underneath it** (not its own top-level section — folded in since it's a single code block, not substantial enough to earn its own tab). The section needs a heading for the TOC (§4.1) to produce an "Overview" entry at all, but an actual visible `## Overview` label reads as redundant directly under the big `<ComponentHeader>` title — resolved by keeping a real `## Overview` heading in the DOM but visually hiding it via `<VisuallyHidden asChild><h2>Overview</h2></VisuallyHidden>` (the same real, tested atom used for icon-only labels elsewhere), placed *before* `<ComponentHeader>` so the TOC's "Overview" entry scrolls all the way to the top of the page, title included. tocbot (the TOC's underlying library) reads heading text directly from the DOM regardless of visibility, so this costs nothing functionally.
...
- **`.storybook/theme.ts`** — a custom `create()`-based Storybook theme (branded colors/type mirroring the primitive tokens) applied to both the manager UI (`.storybook/manager.ts`) and the Docs addon itself (`parameters.docs.theme`).
...
- **`parameters.docs.toc: { headingSelector: "h2" }`** (in `preview.tsx`) — enables the built-in sticky table of contents automatically for every Docs page; no per-component setup needed. **Restricted to `h2` only** (the library default is `h2, h3`) so the TOC reflects just the template's fixed section list (§4's 10 items — note "Import" isn't one of them; it's nested under Overview as an `h3` and deliberately has no TOC entry of its own) — identical across every component — rather than also indexing each component's own `h3` variant subheadings (item 4), which differ per component and would turn the TOC into a per-component variant list instead of a consistent, reusable page nav. This is what makes "the same tab labels on every Docs page" actually true, rather than aspirational — verify it by comparing two different components' rendered TOCs side by side, not just one.
