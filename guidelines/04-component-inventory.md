...
| Breadcrumb | molecule | 🟢 | Compound (`Breadcrumb.Item`/`Link`/`Page`) — a `<nav>` around an ordered list, ending in the current page (`aria-current="page"`); `Breadcrumb.Link` is the `Link` atom (so it takes `external`, `disabled`, `asChild` for a router) with an optional leading `icon`; `size` on the shared scale, a link `tone` (`info` default / `brand` / `neutral`), an optional resting `underline`, a `separator` (chevron that flips in right-to-left text, slash, or anything), `maxItems`/`itemsBeforeCollapse`/`itemsAfterCollapse` collapsing the middle into a "…" button, `labels` (translatable text); a long trail wraps |
