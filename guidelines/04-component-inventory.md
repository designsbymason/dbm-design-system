...
| EmptyState | molecule | 🟢 | Icon + message + optional CTA, used across the system — compound (`EmptyState.Icon`/`Title`/`Description`/`Actions`); `ghost`/`outlined`/`dashed`/`filled` variants, six colour `tone`s (tinting the icon badge), `size` on the shared scale, and `align` (centred or start-aligned); the title is a real heading with a settable `level`; fits inside a `Card` or `Table.Empty` |
