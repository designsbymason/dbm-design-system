...
| Component | Tier | Priority | Notes |
|---|---|---|---|
| Alert | molecule | 🟢 | One component for an inline message and a page-level banner ([ADR-0023](adr/0023-one-alert-with-banner-and-sticky-options-and-a-separate-persistence-hook.md)) — compound (`Alert.Title`/`Description`/`Actions`); five `tone`s (`info`/`success`/`warning`/`danger`/`neutral`) each with its own icon and a default live-region role, `subtle`/`outlined`/`solid` variants, `size` on the shared scale, `banner` (edge to edge), `sticky` (composes `Affix`), `dismissible` with `open`/`defaultOpen`/`onOpenChange` and a fade-and-collapse exit that moves focus on; the companion `usePersistentDismiss` hook (in `primitives`) remembers a dismissal across visits |
