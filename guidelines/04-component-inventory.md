...
| Tabs | molecule | 🟢 | Wraps Radix Tabs — compound (`Tabs.List`/`Trigger`/`Content`); `underline`/`subtle`/`outlined`/`solid` variants, `size` on the shared scale, `rounded`, `orientation` (a breakpoint map turns a vertical list into a horizontal strip on a phone), `activationMode`, `fullWidth`, an `icon` per tab, `asChild` and `forceMount`; a list that is too wide scrolls and keeps the selected tab in view |
| Breadcrumb | molecule | 🟢 | |
