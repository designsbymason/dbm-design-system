...
**Note (`bg.code` superseded):** retired 2026-08-20, at explicit direction — see the dedicated phase below. Every consumer (`docs.css`'s inline-code pill) now uses `bg.info-subtle` directly, which happens to be the exact value `bg.code` itself mirrored in the first place (see this phase's own second bullet). `text.code` and `border.code` are unaffected, still their own separate tokens.
...
- Two more collisions flagged but not fixed, since nothing currently renders either pairing: `border.subtle`/`bg.canvas` in light mode (both now `gray.100`) and `border.subtle`/`bg.neutral-subtle` in dark mode (both now `gray.900`).
...
Full final `bg.*` order after all four changes: `canvas`, `overlay`, `skeleton`, `surface`, `brand` family, `danger` family, `warning` family, `success` family, `info` family, `neutral`, `neutral-strong`, `neutral-strong-hover`, `neutral-subtle`, `neutral-subtle-hover`.
