...
**Nunito and Lora were never actually loaded anywhere until 2026-09-06** (found during Heading's own review, via a real user-reported bug: `weight` visibly changed text for `fontFamily="primary"` but not `"secondary"`) — every component using `fontFamily.secondary` had been silently rendering the browser's own fallback (`Georgia, serif`) the entire time, which is why the effect disappeared specifically there (Georgia traditionally ships only regular/bold, not the full 4-step scale). Storybook now loads both real families at all 4 weights via `.storybook/preview-head.html` (Storybook-preview-only, not shipped in `@dbm-design-system/components` — see `07-storybook-and-documentation-standards.md` §8's own category for this kind of asset). **Still open, not decided by that fix:** whether/how a real consumer of the published package should load these two fonts themselves (a documented recommended `<link>`/`@font-face` snippet, a future docs-site default, or an accepted fallback-stack-only stance) — flagged here rather than decided unilaterally.
...
- **`purple-900` (`#1A133A`) and `purple-950` (`#110C27`) are manual overrides, not generated (2026-09-06, at explicit direction).** Both deviate from what `generate-color-scales.mjs`'s canonical curve would otherwise produce for those two steps (previously `#140B36`/`#040115`, the generated values). Each carries its own `$description` in `color.json` flagging this — **if the generation script is ever re-run, it will silently overwrite both back to the generated values**; carry the override forward by hand (or fold it into the script's own curve/anchor logic first) rather than assuming a re-run is safe. `purple.950` has no current semantic-token consumer; `purple.900` is consumed by `border.brand-subtle` (dark, purple theme) — re-verified against the actual current `bg.brand-subtle` (`gray.950`, per ADR-0011): 1.12:1, still comfortably inside the same deliberate sub-3:1 decorative-accent exception as before (was 1.18:1 pre-override) — no compliance change. See that token's own `$description` in `purple-dark.json` for the full note, including a stale-figure correction (a prior "1.32:1 on gray.900" reading, left uncorrected when ADR-0011 moved that pairing to gray.950) caught while re-verifying this override.
...
| `neutral-subtle` | `gray.50` | `gray.800` | `text.secondary`: 6.59:1 / 7.47:1; `text.primary`: 13.53:1 / 9.66:1; `text.tertiary`: **4.51:1 (razor-thin — revisit if the primitive scale is ever regenerated)** / 5.86:1; `icon.default` (shares `text.tertiary`'s own primitive step): same figures; `text.link` (light): 4.52:1 | AA. Moved from `gray.900` to `gray.800` (ADR-0011). Light-mode `text.tertiary`/`icon.default` figure added 2026-09-04, verified while finalizing `Image` (its fallback state's own `bg.neutral-subtle` + `icon.default` pairing — the fallback icon's actual token, not `text.tertiary`, though the two share the identical value in every theme) — previously unmeasured against this token, shown as unverified "—". |
| `neutral-subtle-hover` | `gray.100` | `gray.700` | `text.secondary`: 6.05:1 / 5.10:1; `text.primary`: 12.41:1 / 6.59:1 (`Table`'s hovered row) | AA. Moved from `gray.800` to `gray.700` (ADR-0011), tracking `bg.neutral-subtle`. |
| `track` | `gray.200` | `gray.700` | vs `bg.surface`: **1.35:1 / 2.05:1** | **Deliberate exception** — fails 3:1, accepted for any track whose own low contrast doesn't block understanding the control (its thumb/fill/handle already carries the boundary/state information), not scoped to passive controls specifically. Used by `ProgressBar`/`ProgressCircle`'s track, and (since [ADR-0016](adr/0016-track-vs-track-strong-scoped-to-decorative-need-not-interactivity.md), 2026-09-15) `Switch`'s and `Slider`'s own tracks too. Light moved `gray.100` → `gray.200` and dark moved `gray.800` → `gray.700` (both 2026-09-15, at explicit direction, once this token became the shared value for `Switch`/`Slider` too — see the token's own `$description`); dark's own prior move (`gray.900` → `gray.800`, ADR-0011) is now superseded by this later one. See "Notable exceptions" below. |
| `track-strong` | `gray.300` | `gray.600` | vs `bg.surface`: **1.72:1 / ~3.00:1** | **Light no longer clears the 3:1 non-text floor; dark sits right at it (razor-thin — revisit if the primitive scale or `bg.surface` itself is ever regenerated).** Moved from `gray.500`/`gray.400` (3.25:1/6.08:1, both AA) to `gray.300`/`gray.600` (2026-09-15, at explicit direction) once `Switch`'s *resting* unchecked state stopped consuming this token (moved to `bg.track`, [ADR-0016](adr/0016-track-vs-track-strong-scoped-to-decorative-need-not-interactivity.md)), leaving `Indicators`' inactive-dot fill (2026-09-04, at explicit direction, in place of `bg.neutral`) as its resting-state consumer, see `guidelines/component-reviews/Indicators.md`. **Flagged, not silently applied:** `Indicators`' own review chose its prior value specifically because a real, clickable dot was judged to need the 3:1 floor, unlike a passive track — this move reopens that question for `Indicators` without a fresh review of its own. **Same day, also became `Switch`'s own unchecked-*hover* state** (replacing `bg.neutral`, which had held that role) — a real but visually subtle one-step darkening in light mode (`gray.200` → `gray.300`), a clearly perceptible one-step lightening in dark mode (`gray.700` → `gray.600`), both confirmed via computed style and live screenshot. |
...
| `brand-subtle` | `purple.50` | `gray.950` (shared, drops brand tint) | `emerald.50` | `gray.950` (shared) | `text.link` (light): 4.50:1 (purple) / 4.52:1 (emerald); `text.brand`/`icon.brand` (dark): 8.29:1 (purple) / 7.22:1 (emerald text) / 9.47:1 (emerald icon) | AA. Dark moved `gray.900` → `gray.950` (ADR-0011) — own pairings improve markedly; now sits at only ~1.1:1 against `bg.surface`, and shares its value with `bg.skeleton`/`border.neutral-subtle` — an accepted trade-off. |
...
| `link` | `blue.600` (shared both brands) | `blue.300` (shared both brands) | vs `bg.surface`: 4.71:1 / 8.20:1 | AA. De-branded (2026-08-20) — identical across all 4 themes, no longer brand-colored. **Never place `text.link` on `bg.canvas`, light or dark** — it measures 4.15:1 (light) / 4.00:1 (dark) there, under the 4.5:1 text floor. `bg.canvas` is reserved for special containers; the ordinary page background is `bg.surface`, which `text.link` is verified against (the figures above), and it is also verified on `bg.neutral-subtle` and `bg.brand-subtle` in light mode (see those rows). Where a link has to sit on the canvas, use a neutral-toned link (`text.secondary`, 6.05:1 / 5.10:1 there) instead. Enforced by convention, not a different token, the same as `text.tertiary` above. (An underline doesn't help — it satisfies WCAG 1.4.1, not the 1.4.3 contrast floor. Moving light mode to `blue.700`, 6.09:1 on the canvas, was considered and not done.) |
...
- **`bg.track`** (`ProgressBar`/`ProgressCircle`'s empty track; also `Switch`'s and `Slider`'s own tracks since [ADR-0016](adr/0016-track-vs-track-strong-scoped-to-decorative-need-not-interactivity.md)) — 1.35:1/2.05:1, fails the 3:1 non-text floor. Accepted deliberately, scoped by whether the track itself must convey the control's boundary/state (it doesn't, here — the thumb/fill already does), not by whether the control is interactive — matching common industry convention (comparable production component libraries' own progress and slider/switch tracks are similarly faint and also unverified against 3:1). `bg.track-strong` (`gray.300`/`gray.600`, 1.72:1/~3.00:1) is no longer a reliably compliant alternative itself — see its own row above and `guidelines/component-reviews/Indicators.md` — so `gray.500`/`gray.400` (3.25:1/6.08:1, its prior value) is the last-known-compliant pairing if a future track-having component's own track genuinely is the boundary-carrying element.
...
**First populated 2026-08-14, during Avatar's own comprehensive review** (`component/avatar.json` holds two token families added together in that single pass, `avatar.size.*` and `avatar.status-size.*`) — the pattern below is now established, not just planned. **Corrected 2026-09-10:** this entry previously claimed a two-day 08-12/08-13 populate-then-extend split; checked against the file's actual git history, both families landed in one commit, 2026-08-14. **Two more files populated since (audited into this doc 2026-08-29, having landed without a narrative entry here at the time):**

- **`component/badge.json` (created 2026-08-15, during Badge's own deep-dive review; revised 2026-08-16 — corrected 2026-09-10, this entry previously attributed the file's origin to 08-16 itself, checked against git history):** two token families, `badge.padding-inline.*` and `badge.size.*` (both `xs`–`xl`). Neither maps onto an existing primitive scale — `size` is a per-step circle/pill diameter computed from real Chrome-measured widest-glyph widths at each step's own font size (so a single character renders as a true circle, not an ellipse), and `padding-inline` is the inline padding that diameter math depends on. Both were revised the following day, at explicit direction (`padding-inline` raised from 5px to a uniform 6px across every step, `size` recomputed to match) — see each token's own `$description` for the full per-step arithmetic; not restated here to avoid the numbers drifting out of sync between two places.
...
- **`component/indicators.json` (2026-09-04, during Indicators' own review):** one token family, `indicators.size.*` (`xs`–`xl` = 4/6/8/10/12px) — the dot diameter Indicators had no `size` prop for at all before this pass. Same reasoning as Avatar's/Badge's/IconButton's own scales: 3 of the 5 requested steps (4, 8, 12px) land on the existing primitive spacing scale (`space.1`/`space.2`/`space.3`) but the other 2 (6, 10px) don't, so a dedicated scale was created rather than distorting the shared one or mixing token references with literals. `md` (8px) matches the component's original hardcoded diameter exactly, so pre-existing usage with no `size` prop is visually unchanged. The active dot's pill width is *not* a second token family — it's derived in CSS via `calc(var(--dot-size) * 3)`, preserving the original 8px/24px ratio at every step without doubling the token surface.
- **`component/tooltip.json` (2026-09-05, during Tooltip's own review — missing from this doc until the 2026-09-07 audit below caught it):** one token family, `tooltip.max-width` (`20rem`) — found hardcoded directly in `Tooltip.module.css` via a package-wide grep for literal values, no home on any existing primitive scale (not a spacing/sizing step, a content-wrapping constraint specific to how wide a tooltip bubble should be allowed to grow). Full context: `guidelines/component-reviews/Tooltip.md`. **Audit note (2026-09-07):** this token and its file were built and shipped correctly at the time — only this doc's own narrative entry and "Files delivered" tree were missed, caught during a full guidelines-vs-repo audit rather than at the time of Tooltip's own review.
- **`component/link.json` (2026-09-07, during Link's own review):** one token family, `link.icon-offset-inline`/`link.icon-vertical-align` — the gap and optical alignment between a Link's text and its trailing external-link icon, previously hardcoded (`0.25em`/`-0.125em`) directly in `Link.module.css`. Unlike every prior component-layer token (all absolute px dimensions), these are deliberately **em-relative** — Link inherits its font size from surrounding context rather than owning a size scale (unlike Button/Tag, which have their own discrete size steps and use `space.*` via `gap` for the equivalent icon spacing), so the icon's offset needs to scale with whatever text size it happens to sit in. The existing `dimension` token type and build pipeline already handled em-relative values correctly (proven first by Heading's own `trim.*` tokens, immediately above) — no pipeline change needed, just the same "component's design genuinely calls for a value with no home on any existing primitive scale" reasoning applied to a relative rather than absolute unit.
- **`component/heading.json` (2026-09-07, adding Heading's `trim` prop):** one token family, `heading.trim.*` (`primary-start`/`primary-end`/`secondary-start`/`secondary-end`) — the negative-margin fallback `trim` uses in browsers without native CSS `text-box-trim` support (Firefox, as of this date). Unlike every other component-layer token so far, these four values aren't measured against a design/pixel grid at all — they're derived from each font's own metrics (`fontBoundingBoxAscent`/`fontBoundingBoxDescent` minus a capital letter's own `actualBoundingBoxAscent`, measured live via Canvas `TextMetrics` against the real loaded webfonts, at a 100px reference size so the ratio scales correctly to any font-size), which is exactly why this needs to be a token pair *per font family* rather than one shared value: Nunito and Lora have genuinely different vertical metrics, so a single number would be wrong for one of them by construction. Verified empirically, not just computed — applying the resulting `-0.30em`/`-0.31em` margins to real rendered text shifted it by the expected pixel amount (`getBoundingClientRect` before/after), confirmed live rather than trusted from the arithmetic alone. Full derivation and the native/fallback split this pairs with: `guidelines/component-reviews/Heading.md`.
- **`component/popover.json` (2026-09-16, building Popover):** one token, `popover.max-width` (`24rem`) — the width cap on Popover's floating content panel. Popover hosts richer, more interactive content than Tooltip's short hint text (a small form, a card), so it gets a wider cap than `tooltip.max-width` (`20rem`) rather than reusing it; like Tooltip's, it's a content-wrapping constraint with no home on any existing primitive scale. Full context: `guidelines/component-reviews/Popover.md`.
- **`component/empty-state.json` (2026-09-19, building EmptyState):** one token, `empty-state.content-max-width` (`28rem`) — the width cap on EmptyState's description text, so a centred message keeps a comfortable line length (roughly 45–75 characters) inside a wide container such as a full page or a wide table cell instead of stretching across it. Like `tooltip.max-width` and `popover.max-width`, it's a content-measure constraint with no home on any existing primitive scale (the spacing scale tops out at `8rem`). Also `empty-state.media-max-width.{xs,sm,md,lg,xl}` (`6`/`8`/`10`/`12`/`15rem`, added the same day for `EmptyState.Media`) — the widest an illustration may grow at each size step, so a large image is scaled down rather than overflowing and each step gets a proportionate one. The first two steps happen to equal `space.24`/`space.32`, but the rest have no home on the spacing scale (which tops out at `8rem`), so the family is dedicated rather than a mix of token references and literals — the same reasoning as `badge.size.*` and `indicators.size.*`. Full context: `guidelines/component-reviews/EmptyState.md`.
...
- **A token's `$description` is baked verbatim into the built CSS as a `/** ... *​/` comment — an unescaped `*/` sequence anywhere in that prose silently corrupts the build.** Found 2026-09-02 (during `AspectRatio`'s review, tracing why `Skeleton` rendered with an invisible background in every live context, across all 4 themes, since 2026-08-25 — see `guidelines/component-reviews/Skeleton.md`). Root cause: `bg.scrim`'s own `$description` in all 4 `packages/tokens/src/semantic/*.json` files contained the literal text `bg.*/text.*` (missing a space) — CSS treats any `*/` as a comment-close token regardless of context, so this prematurely closed `bg.scrim`'s own comment; the resulting stray, unmatched `*/` later in the same declaration list then corrupted the CSS parser's own error-recovery (which discards an invalid declaration up to its next `;`), silently swallowing the entire next declaration (`--dbm-bg-skeleton`) from the parsed stylesheet. Completely invisible to `tsc`/`eslint`/`vitest`/`tsup` — the built CSS is syntactically valid the whole time (comments just end up in the wrong place), so this is only catchable by inspecting the actual browser-computed value of a custom property, not by any static check. **When writing or editing a token's `$description`, never let two adjacent terms produce an unescaped `*/` (e.g. "bg.\*/text.\*", "a\*/b\*")** — add a space, reorder the phrase, or use "and" instead of "/" as the separator.
...
└── component/
    ...
    └── tooltip.json
```
**Updated 2026-08-29** — `icon-button.json` was missing from this tree despite existing on disk since 2026-08-24; see the "Component-layer tokens" section above for its own narrative entry, added the same day. **Updated 2026-09-04** — added `indicators.json`, same day it was created. **Updated 2026-09-07** — added `heading.json` and `link.json`, both the same day they were created; also added `tooltip.json`, which had existed on disk since 2026-09-05 but was missed until a full guidelines-vs-repo audit this same day caught it (see that token's own narrative entry above for why). **Updated 2026-09-20** — added `empty-state.json`, which had been missing from this tree since it was created on 2026-09-19 (its narrative entry above was written at the time); found in a guidelines-vs-repo check.

## Open for next pass
- Component-layer tokens: pattern established (Avatar's size scale) and **confirmed eight times more since (Badge, IconButton, Indicators, Tooltip, Heading, Link, Popover, EmptyState — see "Component-layer tokens" above), not just a one-off** — still extend to other components only as their own reviews turn up a genuine scale gap, not preemptively; the pattern being proven repeatedly doesn't change that it stays reactive, not speculative. Link's own instance also confirmed the pattern extends cleanly to em-relative values, not just absolute px ones.
