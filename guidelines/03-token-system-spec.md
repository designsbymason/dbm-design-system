...
Note: `font-size-xs` currently computes to ~11px, which is below the common 12px practical floor for body-adjacent text — nudge this up when implementing (e.g. floor `xs` at 12px) unless it's intentionally reserved for non-critical micro-labels.

## Contrast verification (done, not assumed)

I ran actual WCAG contrast checks on the semantic pairings rather than eyeballing them, and caught two real failures:
...
**Not yet checked, worth doing before implementation:** icon-on-background pairs (WCAG 1.4.11 non-text contrast, 3:1 threshold), and the full disabled-state / placeholder-text combinations, which typically intentionally sit below AA (that's expected for disabled states, but worth confirming your target compliance level — AA is standard, AAA is stricter and would tighten several of these tertiary/muted pairings).
