...
- **How a consumer loads the CSS is undecided and undocumented (found 2026-09-23, while working out what else to re-export; a Phase 8 item):** two things every component needs are not reachable from a single, documented import. (1) The component styles are compiled into `packages/components/dist/index.css`, but the built `index.js` doesn't import that file and the package's `exports` map only exposes `"."` — so a bundler that honours `exports` may not be able to import the stylesheet at all. (2) The design tokens the components read (`--dbm-*` custom properties) are plain CSS in `@dbm-design-system/tokens` (`css/primitives.css`, `css/component-tokens.css`, and one theme file such as `css/purple-light.css`), which a consumer has to load from that package — a second install and a manual step that no guideline or consumer doc describes. What was checked: the `exports` map, the absence of a CSS import in the built JS, and the token package's `css/*` exports; **not** checked: installing the packed package into a fresh app. Needs a decision before the first publish: expose the stylesheet through `exports` (or import it from the JS), and give one documented way to load the tokens and choose a theme (likely part of the consumer usage guide in §13.1). No fix attempted yet.
- **Whether a Radix-wrapping component reads the page's text direction is undecided (raised 2026-09-25, building `RangeSlider`'s `dir`):** `Tabs`, `Accordion`, `RadioGroup`, `Slider` and `RangeSlider` take an explicit `dir?: "ltr" | "rtl"` passed to Radix and default to left-to-right, and none reads the page. Reading it (Radix's `DirectionProvider`, or measuring the element's computed direction) needs JavaScript, can flash left-to-right on a server-rendered page, and is one decision for every Radix component together — see `05-component-api-conventions.md` §3. A consumer building a right-to-left app passes `dir="rtl"` to each for now.
