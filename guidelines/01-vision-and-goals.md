...
- **The Accessibility addon panel doesn't work in this project's Storybook, for every component, not just one (found 2026-08-16, during Skeleton's finalization review):** `@storybook/addon-a11y`'s panel sits permanently on "Preparing accessibility scan" instead of ever running. Root-caused, not just observed — confirmed via `window.__STORYBOOK_ADDONS_CHANNEL__` that the story-render lifecycle fires correctly (`storyRenderPhaseChanged` completes normally) but none of the addon's own protocol events (`storybook/a11y/request`/`running`/`result`/`error`, found by grepping the installed addon's own bundled source for its `ADDON_ID` constant) ever fire — reproduced identically on Skeleton and the already-finalized Badge, ruling out a component-specific cause. The addon's own bundled UI copy ("This story's component tests failed. Automated accessibility tests will not run until this is resolved.") points to the real cause: in this Storybook version, the panel's automatic scan is gated behind Storybook's **Component Tests** integration (`@storybook/addon-vitest`, the browser-mode Vitest runner that powers the pass/fail status dots next to each story) — this project has never installed it, only `addon-a11y` + `addon-docs` (`.storybook/main.ts`). Without a test-provider reporting any status, the panel has nothing to key off and never leaves its initial state. Doesn't affect real component correctness — `jest-axe` (`02-tech-stack-and-structure.md`'s actual approved a11y-testing tool) is unaffected and is what every component's own test suite already relies on — but the interactive panel itself is currently dead weight for every component, not a Skeleton-specific gap. Fix path: add `@storybook/addon-vitest` (a real dependency decision, same "justify every addition" bar as any other tool) and wire it into `main.ts`. Deliberately not fixed as part of Skeleton's review — infrastructure-level, affects every component, out of scope for a single-component pass.
