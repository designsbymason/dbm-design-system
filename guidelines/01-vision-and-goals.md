...
- **Badge solid variant deferred:** `Badge` currently ships subtle-background only. A solid-fill variant needs `text.on-info`/`on-success`/`on-warning` tokens verified the same way `text.on-danger` was for `Button` in Phase 3 — deliberately scoped out rather than shipped with an unverified color pairing. Build when a solid Badge is actually needed.
