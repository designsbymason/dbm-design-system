...
- **Adding `@storybook/addon-vitest` immediately surfaced 5 real, pre-existing findings across unrelated, already-shipped components (found 2026-08-16, CI-wired same day) — the tool working correctly, not a wiring bug.** `pnpm test:storybook` (249 tests across 50 story files): 244 pass, 5 fail on first run. **None on Avatar, Badge, or Skeleton** — the three finalized components came back completely clean, confirming those review passes actually held.
  ...
  - **`Button`'s instance fixed 2026-08-23, during Button's own `06-engineering-standards.md` §9 review pass — root cause was different from the theory above, not what was originally guessed.** Children were never suppressed; `Button.tsx` computed `isLoading ? (loadingText ?? children) : children`, and `??` only falls through on `null`/`undefined`, not `""`. The Storybook meta defaults `loadingText` to `""` (this project's own "give every controllable prop an explicit value" convention), so the empty string won over `children` on the `Loading` story specifically — reproduced live (spinner-only, no text) before the fix, confirmed showing "Saving" after. Same bug class as Avatar's `identity` chain fix. Changed to `loadingText || children`; the story's `a11y: { test: "todo" }` annotation is removed (it was masking a real, now-fixed defect, not a legitimate exemption) and a regression test added (`Button.test.tsx`). `Textarea`'s "With character count" finding is unrelated and still open, unfixed — Textarea hasn't had its own review pass yet.
