...
- **Phase 5 — Molecules (started 2026-08-09 — earlier than planned):** `Grid`, `GridItem`, `Select` are built. This got ahead of the original sequencing (Phase 5 was meant to start only once Phase 4.9 proved the documentation standard on the full atom tier) — noting the actual order here rather than silently leaving the doc read as strictly sequential. **Tier correction, 2026-09-07 ([ADR-0012](adr/0012-item-components-are-atom-tier-even-when-their-container-is-a-molecule.md)):** `GridItem` is atom-tier as of that date, not molecule — this phase's own history (it was genuinely built/moved during this window) is unaffected, only its current classification. `List` (atom-tier since Phase 3, 2026-07-18) became molecule-tier the same day, for the same reason. **Current state (2026-09-19): 14 of the 36 molecule-tier components are built, 13 of them Finalized** — `Grid`, `List`, `Select`, `CheckboxGroup`, `RadioGroup`, `FormField`, `PasswordInput`, `NumberInput`, `SearchInput`, `Slider`, `Popover`, `Accordion`, `Table` are Finalized; `Card` is built but not yet Finalized. The other 22 (starting with `EmptyState`) follow the itemized dependency order in `04-component-inventory.md`.

**Molecule-build plan set 2026-09-10 (review work not yet started as of this note):** the 3 already-built molecules (`Grid`, `List`, `Select`) — built ahead of schedule during this same phase, per the history above, but never taken through a review pass — get reviewed and Finalized first, one at a time, before any new molecule is started. Each goes through the full `06-engineering-standards.md` §9 review checklist, including the six composition-specific checkpoints added the same day (2026-09-10): compound-component sub-part completeness, atom-reuse audit, consumed-atom defect handling, Radix-primitive prop audit, cross-part ARIA/id wiring, and composed tab order. Once all three are Finalized, the phase moves to building the remaining 33 not-yet-started molecules, one at a time, in the dependency order `04-component-inventory.md`'s own sequencing recommendation lays out — each of those also gets the same full §9 pass (review checklist plus the six composition checkpoints) before being considered done, not a scaled-down version of it. **End state: all 36 molecule-tier components currently listed in `04-component-inventory.md`** (the 33 already tracked, plus `Toolbar`/`Splitter`/`FieldGroup`, added the same day) built and Finalized. **Recount note, 2026-09-14 (corrected 2026-09-18):** `RadioGroup / Radio` split into `RadioGroup` (molecule) and `Radio` (atom) per [ADR-0012](adr/0012-item-components-are-atom-tier-even-when-their-container-is-a-molecule.md), which names this exact pair as a future application of its own standing test. The molecule total stays **36** — `RadioGroup` is still a molecule, only `Radio` moved out (to atom-tier) — and the 33 not-yet-started count above is unchanged. An earlier version of this note recorded 35/32, a miscount; verified against the molecule-tier rows in `04-component-inventory.md`. See that doc's Rough count summary.

**`Grid` Finalized 2026-09-12 — first of the three review-first molecules done.** Full `06-engineering-standards.md` §9 pass (original review plus 8 post-review rounds, see [Grid.md](component-reviews/Grid.md)) complete, including the six composition-specific checkpoints above (the first time they've been exercised — `Grid` is a plain layout primitive with no compound sub-parts, no consumed atoms, and no wrapped Radix primitive, so most of the six passed trivially by having nothing to check, noted explicitly rather than silently skipped).

**`List` Finalized 2026-09-13 — second of the three review-first molecules done.** Full `06-engineering-standards.md` §9 pass complete (see [List.md](component-reviews/List.md)), including two real, confirmed bugs found and fixed during the review itself, not just checklist gaps: a `{...props}`-spread-ordering accessibility bug (a consumer's own `role` could silently override `List`'s Safari/VoiceOver `role="list"` fix) and a genuinely inert new `type` prop (reached the DOM correctly but CSS `list-style-type` always overrode it, fixed by translating `type` into its own CSS equivalent instead of relying on the native attribute alone).

**`Select` Finalized 2026-09-14 — third of the three review-first molecules done.** Full `06-engineering-standards.md` §9 pass complete (original review plus twelve post-review fix rounds and a second final-review pass, see [Select.md](component-reviews/Select.md)) — the first molecule review where the compound-component and Radix-primitive checkpoints (added 2026-09-10, never yet exercised on `Grid`/`List`) actually apply: `Select.Option` is a real dot-notation sub-part, and `Select` wraps Radix Select. Found and fixed the same `{...props}`-ordering accessibility bug class as `List` (this time on `aria-invalid`), a real compound-sub-part gap (`Select.Option` couldn't accept `id`/`style`/`data-testid`/any native passthrough), added `side`/`align` (matching already-Finalized `Tooltip`'s own precedent), `asChild` support with a dedicated `trigger` prop — a real architectural decision (asked rather than guessed) resolving a `children`-already-used-for-options conflict `Button`'s own `asChild` precedent didn't directly cover — and `onClear`. The post-review rounds also produced this codebase's first compound-sub-part Properties table (`Select.Option`'s own, via a docs-only `tags: ["!dev"]` stories file) and a broadened standing rule (`07-storybook-and-documentation-standards.md` §4) against decision-point/internal-process language in any Docs page, not just literal doc-path citations. With all three review-first molecules now Finalized, the phase moves to building the remaining 33 not-yet-started molecules next, per the plan above (`RadioGroup` is one of the 33; `Radio`, split out to atom-tier the same day per the note above, needed building first, ahead of it).
