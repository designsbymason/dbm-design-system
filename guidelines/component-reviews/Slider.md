...
**Found while building `RangeSlider`** (details in [RangeSlider.md](RangeSlider.md)): two defects, fixed the same day at explicit direction (below), and one open question — the slider does not mirror in right-to-left text. A third defect, reported afterwards, is fixed in the last section.
