...
This folder exists to keep `07-storybook-and-documentation-standards.md` §6 from growing without bound as more components (currently 66 of a planned 108, updated 2026-09-23 — see that section's own table for the current, authoritative count rather than trusting this number to stay current) go through review — that section stays a compact, scannable status table; the actual per-component narrative lives here instead. See `CLAUDE.md`'s note on numbered docs staying current-state reference, applied to this specific unbounded-growth case.
