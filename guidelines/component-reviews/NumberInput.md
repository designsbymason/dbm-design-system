...
Same review as `Card`'s and `Table`'s (standard: `07-storybook-and-documentation-standards.md` §4.2). Findings: every snippet spelled out ~14 props (`hasError={false}`, `readOnly={false}`, `name=""`, an empty `aria-label=""`) and a no-op `onValueChange`; four stories ("No min/max", "Decimal step", "Error state", "Disabled") were the Playground plus one arg, with the one arg lost in the noise; and "With a clear button" froze `value={5}` with a no-op `onClear`, so the clear button it demonstrated couldn't clear. Every story now sets `parameters.docs.source.code` to a hand-written snippet from the new `NumberInput.snippets.ts`; the four one-arg stories use the Playground's builder (so "No min/max" is just the value with no bounds), and "With a clear button" shows real state with `onClear={() => setValue(undefined)}`. An empty `aria-label` in the Playground is written as a real one. All snippets and Playground combinations were typechecked against the real types (the controlled ones with the real `useState` lines, so the state handlers' types were checked too). **Finalized status unchanged** — story-file and docs-only, no component code, props, or tokens touched.
