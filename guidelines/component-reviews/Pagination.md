...
4. **`compact="container"` doesn't re-measure when the digits change.** Measured in real Chromium: a box 441px wide (nav 439px), `pageCount={5000}`. At page 1 the row is 422px and fits; on page 2000 its numbers are four digits wide and it is 449px, but it stays expanded and overflows its box by 10px, because the measurement effect re-runs on a change of slot count or size, not of what the slots show. The same staleness applies the other way: collapsed, it never learns the row got narrower. It only bites when the container sits within a few tens of pixels of the row's width, on a page count large enough to change digits. **The obvious fix has a catch:** re-measuring on every page change makes a *click on a page number* able to collapse the row, and the button being pressed is then `display: none`, so keyboard focus falls to the page. Recommended: re-measure whenever the row changes, but hold a collapse back while focus is on one of the numbers, applying it when focus leaves. Not built here because it changes what the component does under keyboard focus, which is worth a nod first.

**Named gap, not built (in line with `06` §7's "no translation system for v1"):**

5. **The visible page numbers can't be localised.** `labels.page` translates the accessible name ("Página 5"), but the digit shown is always the raw number, so a locale with its own numerals would have a name that no longer contains the visible text (label-in-name). Fine for every locale that uses Western digits, which is what the Docs' translated example does. A `formatNumber`-style option would close it; it is new API, so it is a proposal, not a fix.
