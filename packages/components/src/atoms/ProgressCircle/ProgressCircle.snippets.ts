// The code shown under each story's "Show code" button on ProgressCircle's Docs page.
//
...
// and — for "With custom value label" — shows `formatValueLabel={() => {}}` in place
// of the real formatter, hiding the one thing that story demonstrates. Each snippet
...
import type { ProgressCircleSize, ProgressCircleTone } from "./ProgressCircle.types";

export const progressCircleSnippets = {
  allTones: `{/* tone: "brand" (default) | "info" | "success" | "warning" | "danger" */}
<ProgressCircle tone="success" label="success" value={65} />`,

  allSizes: `{/* size: "xs" | "sm" | "md" (default) | "lg" | "xl" */}
<ProgressCircle size="lg" label="Size lg" value={65} />`,
...
export interface ProgressCirclePlaygroundSnippetArgs {
  value?: number;
  max?: number;
  ...
  /** Playground-only: leaves `value` out, which is what makes a progress circle indeterminate. */
 ...
 * change a few args (a value label, `aria-valuetext`, indeterminate). A progress circle
 * needs an accessible name, so the `label` is always written.
 */
export function progressCirclePlaygroundSnippet(args: ProgressCirclePlaygroundSnippetArgs): string {
  ...
  return `<ProgressCircle ${attributes.join(" ")} />`;
}
